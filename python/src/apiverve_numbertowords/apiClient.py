...
class NumbertowordsAPIClientError(Exception):
...
class ValidationError(NumbertowordsAPIClientError):
...
class NumbertowordsAPIClient:
    ...
    VALIDATION_RULES = {"number": {"type": "number", "required": True}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the NumbertowordsAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises NumbertowordsAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise NumbertowordsAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[NumbertowordsAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns NumbertoWordsData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises NumbertowordsAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise NumbertowordsAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise NumbertowordsAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise NumbertowordsAPIClientError(f"Request failed: {str(e)}")
